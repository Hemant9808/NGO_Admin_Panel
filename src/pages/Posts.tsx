...
const Events: React.FC = () => {
  ...
  const DeletePost = async (post_id: string) => {
    ...
    try {
      const response = await axios.delete(`https://localhost:3000/deletePost/${post_id}`, {
  ...
  const fetchPosts = async () => {
    try {
      ...
      return posts; // You can use the posts data in your frontend component
    } catch (error) {
      ...
      <div className="p-8 px-[5vw] z-[5] max-w-[1300px] flex flex-col justify-center">
        <div className="mb-6 flex flex-col md:flex-row justify-between sm:gap-8 items-center">
          ...
          <div data-aos={shouldAnimate ? 'slide-left' : ''} className="flex flex-col sm:flex-row items-center gap-2 sm:gap-12 mt-2 w-full md:w-auto">
            ...
            <Link to="/createPost" className="bg-blue-500 text-white py-2 px-4 rounded-md hover:bg-blue-600 mt-2 sm:mt-0 w-full sm:w-auto text-center">Create Event</Link>
